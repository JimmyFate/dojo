...
trait IWorld<T> {
    ...
    fn model(self: @T, name_hash: felt252) -> (ClassHash, ContractAddress);
    ...
    fn emit(self: @T, model: felt252, keys: Array<felt252>, values: Span<felt252>);
...
mod world {
    ...
    impl World of IWorld<ContractState> {
        ...
        fn set_metadata(ref self: ContractState, metadata: ResourceMetadata) {
            ...
            let model = Model::<ResourceMetadata>::name_hash(@metadata);
        ...
        fn register_model(ref self: ContractState, class_hash: ClassHash) {
            ...
            let (address, name_hash) = dojo::model::deploy_and_get_name_hash(
            ...
            if name_hash.is_zero() || self.deployed_contracts.read(name_hash).is_non_zero() {
                panic_with_felt252(Errors::INVALID_MODEL_NAME);
            }

            // If model is already registered, validate permission to update.
            let (current_class_hash, current_address) = self.models.read(name_hash);
            if current_class_hash.is_non_zero() {
                assert(self.is_owner(caller, name_hash), Errors::OWNER_ONLY_UPDATE);
            ...
            self.models.write(name_hash, (class_hash, address));
            EventEmitter::emit(
                ref self,
                ModelRegistered { name_hash, prev_address, address, class_hash, prev_class_hash }
            );
        }
        ...
        /// # Arguments
        ///
        /// * `name_hash` - The keccak(name) of the model.
        ///
        /// # Returns
        ///
        /// * (`ClassHash`, `ContractAddress`) - The class hash and the contract address of the model.
        fn model(self: @ContractState, name_hash: felt252) -> (ClassHash, ContractAddress) {
        ...
        fn emit(
            self: @ContractState, model: felt252, mut keys: Array<felt252>, values: Span<felt252>
        ) {
            let contract = get_caller_address();
            contract.serialize(ref keys);
